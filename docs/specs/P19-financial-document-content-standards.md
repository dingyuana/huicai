...
### 1. 输入契约
→ 见本文 [## 3. 各模块详细规范（字段定义/VO 结构/API 规范）](#3-各模块详细规范)

### 2. 输出契约
→ 见本文 [## 4. 验收标准（跨模块 AC1-AC14）](#4-验收标准-跨模块)

### 3. 状态流转
→ 见本文 [### 2.4 状态机标准（业务单据/凭证/费用报销/应收应付状态机）](#24-状态机标准-已实现)

### 4. 异常处理
→ 见本文 [## 5. 常见错误及预防（BusinessException 抛出点）](#5-常见错误及预防)
...
### 场景 1：业务单据摘要包含对方单位信息
**Given** 一条业务单据，其 `supplierId` 或 `customerId` 已设置
**When** 调用 `enrichSummary()` 生成展示摘要
**Then** 返回的摘要格式为 `{付/收}{对方单位名称}-{原始摘要}`，且 `enrichedSummary` 字段非空

### 场景 2：科目代码缺失时拒绝生成空凭证
**Given** 一条业务单据对应的科目代码在数据库中不存在
**When** 调用 `generateVoucher()` 生成凭证
**Then** 系统抛出 `BusinessException`，提示"科目代码不存在"，事务回滚，不生成空凭证

### 场景 3：摘要不符合规范时创建被拦截
**Given** 用户创建一张新凭证，摘要仅包含系统流水号
**When** 提交创建请求
**Then** 系统校验失败，返回错误信息"摘要不可仅包含系统流水号"
