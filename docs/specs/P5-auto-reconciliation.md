...
## 1. 输入契约
→ 见本文 三、接口设计（3.1 新增 API 列表、3.2 请求/响应示例）

## 2. 输出契约
→ 见本文 五、验收标准（9 条验收条件）

## 3. 状态流转
→ 见本文 二、功能规格（FR-RECON-03 银行流水自动匹配流程：分类→创建单据→推荐→自动/手动核销）

## 4. 异常处理
→ 见本文 二、功能规格（FR-RECON-04 匹配策略中的容差/尾差处理、反核销 rollback）
...
### 场景 1：收款核销推荐返回匹配列表
**Given** 有一笔收款单 10000 元，对应客户 A 有待核销发票  
**When** 调用 POST /api/v1/reconciliation/receipt/{id}/recommend  
**Then** 返回匹配度 ≥ 0.7 的发票候选列表  
**And** 按匹配度降序排列，含评分和匹配等级

### 场景 2：高匹配度一键核销
**Given** 匹配度 ≥ 0.95  
**When** 执行核销操作  
**Then** 自动创建核销单并确认  
**And** 写入 t_reconciliation_log 审计日志

### 场景 3：反核销回滚金额
**Given** 一笔已确认的核销记录  
**When** 调用 POST /api/v1/reconciliation/{id}/reverse  
**Then** 恢复发票未核销余额  
**And** 日志状态变为 CANCELLED