...
| 提取比例 | BigDecimal | 盈余公积提取比例，默认 0.10 |
...
| 转换 | 前置条件 | 副作用 |
|------|---------|--------|
| 净利润→利润分配 | 期末损益结转完成 | 生成分配凭证（借:本年利润/贷:利润分配-未分配利润） |
| 利润分配→盈余公积 | 提取比例>0 | 生成提取凭证（借:利润分配-盈余公积/贷:盈余公积） |
| 已分配→禁止重分配 | 存在利润分配凭证 | BusinessException |
