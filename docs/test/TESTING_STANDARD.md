...
**示例流程（销售流程）**：
```
创建销售发票 → 审核发票 → 生成应收单 → 收款核销 → 生成记账凭证
```

**必须验证的一致性**：
1. 状态流转正确性（PENDING_CONFIRM → CONFIRMED → VOUCHERED）
2. 金额一致性（发票总金额 = 应收金额 = 凭证借贷金额）
3. 外键关联正确性（发票 ID = 应收单 docId）
...
| 应付单 (Payable) | `period` | varchar(6) 长度限制，不能用 `"2026-06"` | 2026-06-27 |
| 应付单 (Payable) | `vendorId` | 外键约束 (必须存在于 t_customer) | 2026-06-27 |
...
| 表 | 依赖表 | 备注 |
|---|---|---|
| t_receivable | t_customer | customerId 外键 |
| t_payable | t_vendor | vendorId 外键 |
...
**测试数据准备顺序**：`t_subject` → `t_customer/t_vendor` → 业务表
