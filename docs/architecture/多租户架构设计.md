...
> **版本**：V1.0 | **最后修改**：2026-07-23 | **作者**：Hermes
> **状态**：📋 草案
...
> **状态字段语义对齐说明**（2026-07-23 修正）：各表 `status` 字段语义不同，详见 S-26 SPEC §3.1：
> - **t_agency / t_enterprise**：遵循企业状态机 4 态（PENDING/ACTIVE/SUSPENDED/TERMINATED），与 S-26 YAML 契约 transitions T-01~T-04 一致
> - **t_user**：用户登录态（ACTIVE/INACTIVE/LOCKED），与 S-02 §3.1 用户状态机一致，不参与企业状态机转换
> - **t_agency_enterprise**：绑定关系状态（ACTIVE/INACTIVE），仅表达代理与客户的绑定有效性，非企业状态机

### 2.2 用户类型说明

| 用户类型 | agency_id | enterprise_id | 可操作的公司 |
|---------|-----------|--------------|-------------|
| `SUPER_ADMIN` | null | null | 所有（系统管理） |
| `AGENCY` | 1 | null | 该代理绑定的所有客户 |
