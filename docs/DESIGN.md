...
> **2026-06-25 修订**：B 类业务收付款无匹配应收应付单时直接落终态 `CONFIRMED`，C 类兜底直接落 `CONFIRMED`，核销工作台触发源改为收款单/付款单。

```
┌──────────────────────────────────────────────────────────────────┐
│ Phase 1: 文件导入                                                  │
│  上传 Excel/CSV → 列名智能识别(两阶段+级联回退) → 解析入库          │
│  默认 review_status = 'classified' (V53)                           │
└──────────────────────────────────────────────────────────────────┘
                            ↓
┌──────────────────────────────────────────────────────────────────┐
│ Phase 2: 智能分类                                                  │
...
│  确认(批量) / 驳回 / 重分类                                         │
│  终态标签: voucher_generated/payment_created/CONFIRMED=绿色         │
│           中间态: classified/PENDING/RECLASSIFIED/manual_pending=橙色│
└──────────────────────────────────────────────────────────────────┘
                            ↓ 确认后
┌──────────────────────────────────────────────────────────────────┐
│ Phase 4: A/B/C 路由                                                │
│  A类 (bank_fee/interest/tax/social_security/insurance_fee)         │
│    → 直接生成凭证草稿(DRAFT) → 终态 voucher_generated               │
│  B类 (business_receipt/payment)                                    │
│    → 精确匹配应收/应付单 → 生单+核销提示 → 终态 payment_created     │
│    → 无匹配应收/应付单 → 直接落终态 CONFIRMED（不再生单）           │
│    → 生单失败 → 直接落终态 CONFIRMED                                │
│  B类 (internal_transfer/salary_payment 等)                         │
│    → 生成银行转账单/付款单 → 终态 payment_created                   │
│  C类 (pending/不明确)                                              │
│    → 兜底分类 → 直接落终态 CONFIRMED，由用户后续手工处理            │
│  核准: voucher_generated/payment_created → approve() → approved     │
│  重分类: payment_created → review(reclassify) → RECLASSIFIED       │
└──────────────────────────────────────────────────────────────────┘
                            ↓ 核销
┌──────────────────────────────────────────────────────────────────┐
│ Phase 5: 核销工作台（收款单/付款单触发）                             │
│  核销触发源: 收款单 tab / 付款单 tab（银行流水不直接参与核销）      │
│  点击行 → 核销推荐(金额+摘要+对方名多维评分) → 执行核销             │
│  核销审批: CONFIRMED → EXECUTED（人工审批）                         │
└──────────────────────────────────────────────────────────────────┘
```

### 13.2 销售发票完整流

> **2026-06-25 修订**：发票导入时只创建发票记录（PENDING_CONFIRM），业务单和应收单在发票审核确认后才异步创建。硬约束：所有审核/结转必须由人确定，系统不允许自动调整状态。
...
│  → 写入 t_output_invoice (PENDING_CONFIRM)                          │
│  → 不创建业务单和应收单（审核后才创建）                               │
│  → 所有发票导入完成后自动触发批量红冲关联                              │
