...
test.describe('菜单导航', () => {
  for (const menu of MENUS) {
    test(`展开【${menu.name}】并点击第一个子菜单`, async ({ page }) => {
      ...
      // 等待子菜单出现
      await page.waitForSelector('.el-menu--inline', { timeout: 5_000 });

      // 点击第一个子菜单项
      const firstSub = page.getByRole('menuitem', { name: menu.firstSubMenu });
      await expect(firstSub).toBeVisible({ timeout: 5_000 });
      await firstSub.click();
      await page.waitForLoadState('domcontentloaded');

      // 验证页面加载成功（主内容区域可见）
      await expect(page.locator('.el-main')).toBeVisible({ timeout: 10_000 });
      // 验证无 500 错误提示
