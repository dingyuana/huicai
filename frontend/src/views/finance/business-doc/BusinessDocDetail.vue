    ...
    <el-card shadow="never">
      <div class="page-header">
        ...
        <div>
          ...
          <el-button v-if="doc?.status === 'APPROVED' && !doc?.voucherNo" type="warning" @click="onGenerateVoucher">生成凭证</el-button>
      ...
      <template v-else-if="reconResult">
        ...
        <el-table v-if="reconResult.items?.length" :data="reconResult.items" border stripe size="small">
          ...
          <el-table-column label="操作" width="120" align="center" fixed="right">
            <template #default="{ row }">
              <el-button text size="small" type="primary" :loading="row._executing" @click="onExecuteRecon(row as any)">执行核销</el-button>
            </template>
          </el-table-column>
        </el-table>
