...
import {
  ...
  clearAll, clearReceivables, clearPayables, clearSettlements, clearReportData
...
const statItems = [
  ...
  { key: 'reportData', label: '报表数据', icon: DataBoard, color: 'blue' },
]
...
const normalOps: Array<{ type: string; title: string; desc: string; icon: any; color: string; btnType: 'success' | 'warning' | 'info' | 'primary' | 'danger'; btnText: string }> = [
  ...
  { type: 'reportData', title: '清空报表数据', desc: '删除科目余额表与现金流量表数据(资产负债表/利润表/现金流量表清零; 仅开发用)', icon: DataBoard, color: 'orange', btnType: 'warning', btnText: '清空' },
...
async function onClear(type: string) {
  try {
    ...
    else if (type === 'reportData') res = await clearReportData()
