  ...
  <div class="payable-list">
    <el-card shadow="never">
      <div class="page-header">
        <span class="page-title">应付核销</span>
        <el-alert title="本页聚焦应付款项的付款状态，如需编辑单据请前往「财务核心 → 业务单据」" type="info" :closable="false" size="small" style="margin-bottom:16px" />
      </div>

      <el-form :model="query" inline class="filter-form">
        <el-form-item label="供应商">
          <el-select v-model="query.vendorId" filterable clearable placeholder="全部供应商" style="width:200px" @change="fetchData">
      ...
      <el-table :data="list" v-loading="loading" border :row-class-name="rowClassName">
        ...
        <el-table-column label="应付单编号" width="140">
          <template #default="{ row }">
            <span style="font-family:monospace;color:#409EFF">{{ row.payableNo || ('#' + row.id) }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="应付总额" width="120" align="right">
    ...
    <!-- 详情对话框 -->
    <el-dialog v-model="detailVisible" title="应付核销详情" width="640px">
...
const query = reactive({ vendorId: undefined as number | undefined, period: '', current: 1, size: 20 })
const list = ref<any[]>([])
const total = ref(0)
...
const fetchData = async () => {
  ...
  try {
    ...
    const res: any = await pagePayable(params)
...
onMounted(async () => {
  vendors.value = (await listVendor()) as any[]
  fetchData()
})
</script>
