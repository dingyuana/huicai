...
public class CustomerServiceImpl implements CustomerService {
    ...
    private static final List<String> TEMPLATE_HEADERS = List.of(
            "客户编码", "客户名称", "联系人", "电话", "邮箱", "地址", "信用额度", "账期(天)", "备注"
    );
    ...
    private static final int COL_CREDIT_LIMIT = 6;
    private static final int COL_CREDIT_DAYS = 7;
    ...
    public ImportResult importFromExcel(MultipartFile file) {
        ...
        List<CustomerEntity> validEntities = new ArrayList<>();

        // 加载数据库中已有的客户编码，用于校验唯一性
        Set<String> existingCodes = new HashSet<>();
        mapper.selectList(new LambdaQueryWrapper<CustomerEntity>().select(CustomerEntity::getCode))
        ...
        try (InputStream is = file.getInputStream(); Workbook workbook = WorkbookFactory.create(is)) {
            ...
            for (int rowIdx = 1; rowIdx <= lastRowNum; rowIdx++) {
                ...
                try {
                    CustomerEntity entity = parseRow(row, existingCodes);
                    if (entity != null) {
            ...
            if (!validEntities.isEmpty()) {
                for (CustomerEntity entity : validEntities) {
                    mapper.insert(entity);
                }
                log.info("Excel 导入客户完成，成功导入 {} 条", validEntities.size());
            }
    ...
    public InputStream createExportTemplate() {
        try (XSSFWorkbook workbook = new XSSFWorkbook()) {
            Sheet sheet = workbook.createSheet("客户模板");
            ...
            sheet.setColumnWidth(COL_CREDIT_LIMIT, 4000);
            sheet.setColumnWidth(COL_CREDIT_DAYS, 3000);
            sheet.setColumnWidth(COL_REMARK, 6000);

            // 写入一行示例数据
            Row exampleRow = sheet.createRow(1);
            String[] exampleData = {"C001", "示例客户", "张三", "13800138000", "zhangsan@example.com", "北京市朝阳区", "50000", "30", "一般客户"};
     ...
     * 解析单行数据
     */
    private CustomerEntity parseRow(Row row, Set<String> existingCodes) {
        ...
        String creditLimitStr = getCellStringValue(row.getCell(COL_CREDIT_LIMIT));
        String creditDaysStr = getCellStringValue(row.getCell(COL_CREDIT_DAYS));
        ...
        if (StrUtil.isNotBlank(creditLimitStr)) {
            try {
                creditLimit = new BigDecimal(creditLimitStr);
            } catch (NumberFormatException e) {
                throw new IllegalArgumentException("信用额度格式不正确: '" + creditLimitStr + "'");
            }
        }

        // 解析账期
        Integer creditDays = 30;
        if (StrUtil.isNotBlank(creditDaysStr)) {
            try {
                creditDays = Integer.parseInt(creditDaysStr);
            } catch (NumberFormatException e) {
                throw new IllegalArgumentException("账期(天)格式不正确: '" + creditDaysStr + "'");
            }
        }
