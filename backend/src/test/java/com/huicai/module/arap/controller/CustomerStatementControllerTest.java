...
import com.huicai.module.arap.entity.CustomerStatementEntity;
import com.huicai.module.arap.service.CustomerStatementService;
import com.huicai.module.arap.service.CustomerStatementService.DisputeRequest;
...
class CustomerStatementControllerTest {
    ...
    void generateStatements_paramsBound() throws Exception {
        ...
        mvc.perform(post("/api/v1/customer-statements/generate")
    ...
    void sendStatement_returnsOk() throws Exception {
        mvc.perform(post("/api/v1/customer-statements/1/send"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200));
    }
    ...
    void confirmStatement_returnsOk() throws Exception {
        mvc.perform(post("/api/v1/customer-statements/1/confirm"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200));
    }
    ...
    void disputeStatement_paramsBound() throws Exception {
        ...
        mvc.perform(post("/api/v1/customer-statements/1/dispute")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void pageStatements_defaultParams() throws Exception {
        mvc.perform(get("/api/v1/customer-statements/page"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200));
    }
    ...
    void resolveOutstandingItem_returnsOk() throws Exception {
        mvc.perform(post("/api/v1/outstanding-items/1/resolve"))
                .andExpect(status().isOk());
    }
}