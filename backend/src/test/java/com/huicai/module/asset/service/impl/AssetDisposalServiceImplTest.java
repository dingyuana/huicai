...
class AssetDisposalServiceImplTest {
    ...
    @Mock private AssetDisposalMapper assetDisposalMapper;
    ...
    void getById_存在_返回Entity() {
        when(assetDisposalMapper.selectById(1L)).thenReturn(stubEntity());
        AssetDisposalEntity result = service.getById(1L);
        assertNotNull(result);
        verify(assetDisposalMapper).selectById(1L);
    }

    @Test
    void getById_不存在_抛BusinessException() {
        when(assetDisposalMapper.selectById(99L)).thenReturn(null);
        assertThrows(BusinessException.class, () -> service.getById(99L));
    }

    @Test
    void pageQuery_调selectPage() {
        when(assetDisposalMapper.selectPage(any(), any())).thenReturn(null);
        service.pageQuery("PENDING_APPROVAL", 1, 20);
        verify(assetDisposalMapper).selectPage(any(), any());
    }
}
