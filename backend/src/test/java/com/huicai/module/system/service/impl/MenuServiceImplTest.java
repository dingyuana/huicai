...
class MenuServiceImplTest {
    ...
    void getMenuTree_调selectList() {
        when(menuMapper.selectList(any())).thenReturn(Collections.singletonList(stubEntity()));
        service.getMenuTree();
        verify(menuMapper).selectList(any());
    }

    @Test
    void getRoutesByUserId_调selectList() {
        when(userRoleMapper.getRoleIdsByUserId(1L)).thenReturn(java.util.Collections.emptyList());
        service.getRoutesByUserId(1L);
        verify(userRoleMapper).getRoleIdsByUserId(1L);
    }
}