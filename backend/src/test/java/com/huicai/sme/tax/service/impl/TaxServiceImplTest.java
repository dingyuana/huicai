...
class TaxServiceImplTest {
    ...
    // ========== 进项发票 pageQueryInput BDD ==========

    @Test
    @SuppressWarnings({"unchecked", "rawtypes"})
    void pageQueryInput_scopeCompleted_SQL含statusIN且参数含全部终态() {
        // given
        com.baomidou.mybatisplus.core.metadata.TableInfoHelper.initTableInfo(
                new org.apache.ibatis.builder.MapperBuilderAssistant(
                        new com.baomidou.mybatisplus.core.MybatisConfiguration(), ""), InputInvoiceEntity.class);
        Page<InputInvoiceEntity> page = new Page<>(1, 20, 1);
        page.setRecords(new ArrayList<>());
        when(inputMapper.selectPage(any(), any())).thenReturn(page);

        // when — completed 视图必带终态过滤
        service.pageQueryInput(null, null, null, "completed", null, null, 1, 20);

        // then — 正向: SQL 含 status IN 且参数含全部终态
        ArgumentCaptor<com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper> captor =
                ArgumentCaptor.forClass(com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper.class);
        verify(inputMapper).selectPage(any(), captor.capture());
        String sql = (String) captor.getValue().getSqlSegment();
        assertTrue(sql.contains("status IN"), "completed 应生成 status IN: " + sql);
        assertFalse(sql.contains("status NOT IN"), "completed 不应生成 status NOT IN: " + sql);
    }
    ...
    @Test
    @SuppressWarnings({"unchecked", "rawtypes"})
    void pageQueryInput_scopePending_排除全部终态() {
        // given
        com.baomidou.mybatisplus.core.metadata.TableInfoHelper.initTableInfo(
                new org.apache.ibatis.builder.MapperBuilderAssistant(
                        new com.baomidou.mybatisplus.core.MybatisConfiguration(), ""), InputInvoiceEntity.class);
        ...
        // when — pending 视图排除终态
        service.pageQueryInput(null, null, null, "pending", null, null, 1, 20);
    ...
    @Test
    @SuppressWarnings({"unchecked", "rawtypes"})
    void pageQueryInput_带日期范围_生成invoiceDate区间条件() {
        // given
        com.baomidou.mybatisplus.core.metadata.TableInfoHelper.initTableInfo(
                new org.apache.ibatis.builder.MapperBuilderAssistant(
                        new com.baomidou.mybatisplus.core.MybatisConfiguration(), ""), InputInvoiceEntity.class);
        ...
        // when — completed 视图必带日期
        service.pageQueryInput(null, null, null, "completed",
                java.time.LocalDate.of(2026, 1, 1), java.time.LocalDate.of(2026, 8, 31), 1, 20);
